...
class H3CMechanismDriver(driver_api.MechanismDriver):
    ...
    def update_port_postcommit(self, mech_context):
        ...
        if ('port_extensions' in current_port and
                'member_status' in current_port['port_extensions']):
            from neutron_lbaas.db.loadbalancer import loadbalancer_db
            member_status = current_port['port_extensions']['member_status']
            loadbalancer = loadbalancer_db.LoadBalancerPluginDb()
            assoc_qry = context.session.query(loadbalancer_db.Member)
            assoc = \
                assoc_qry.filter_by(
                    tenant_id=member_status['tenant_id'],
                    address=member_status['ip_address']).first()
            if assoc is not None:
                if member_status['action'] == 'power_on':
                    status = "ACTIVE"
                    loadbalancer.update_status(context, loadbalancer_db.Member,
                                               assoc['id'], status)
                else:
                    status = "INACTIVE"
                    loadbalancer.update_status(context, loadbalancer_db.Member,
